...
describe('retryableEmbedMany', () => {
  describe('success', () => {
    ...
    it('should hand back the SDK result untouched', async () => {
      ...
      // Act
      const direct = await embedMany({ model, values });
      const wrapped = await retryableEmbedMany({ model, values });
      ...
      expect(wrapped.embeddings.length).toBe(2);
    });
  });
  ...
  describe('result-based retries', () => {
    it('should fall over on too few embeddings', async () => {
      ...
      await retryableEmbedMany({
        ...
        values: ['hi'],
        retry: [
          embeddingResult((res) => res.embeddings.length < 2).switch({
            model: fallback,
          }),
        ],
      });
