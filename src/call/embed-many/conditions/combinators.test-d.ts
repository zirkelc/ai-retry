...
import { and, error, httpStatus, not, or, result } from './index.js';

const fallback = openai.textEmbedding('text-embedding-3-small');

describe('top-level combinators (call/embed-many)', () => {
  ...
  it('carries the entry point commit result into a result retryable', () => {
    expectTypeOf(
      result((res) => res.embeddings.length === 0).switch({ model: fallback }),
    ).toEqualTypeOf<
      CallRetryable<ResolvableEmbeddingModel, never, EmbedManyCommitResult>
    >();
  });
