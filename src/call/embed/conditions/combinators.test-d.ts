...
describe('top-level combinators (call/embed)', () => {
  ...
  it('carries the entry point commit result into a result retryable', () => {
    expectTypeOf(
      result((res) => res.embedding.length === 0).switch({ model: fallback }),
    ).toEqualTypeOf<
      CallRetryable<ResolvableEmbeddingModel, never, EmbedCommitResult>
    >();
  });
