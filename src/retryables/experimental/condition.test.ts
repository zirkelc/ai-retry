...
describe('Condition', () => {
  ...
  describe('and', () => {
    it(`should match only when both conditions match`, async () => {
      // Arrange
      ...
      // Act
      const both = await truthy.and(truthy).evaluate(ctx);
      const mixed = await truthy.and(falsy).evaluate(ctx);
  ...
  describe('or', () => {
    it(`should match when either condition matches`, async () => {
      // Arrange
      const truthy = new Condition<MockLanguageModel>(() => true);
      const falsy = new Condition<MockLanguageModel>(() => false);
      const ctx = buildErrorContext(new Error('boom'));
      ...
      // Act
      const either = await falsy.or(truthy).evaluate(ctx);
      const neither = await falsy.or(falsy).evaluate(ctx);
  ...
  describe('not', () => {
    it(`should invert the wrapped predicate`, async () => {
      // Arrange
      const truthy = new Condition<MockLanguageModel>(() => true);
      const falsy = new Condition<MockLanguageModel>(() => false);
      const ctx = buildErrorContext(new Error('boom'));
      ...
      // Act
      const inverted = await truthy.not().evaluate(ctx);
      const doubled = await falsy.not().not().evaluate(ctx);
