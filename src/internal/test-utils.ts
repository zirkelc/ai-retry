...
/**
 * Test-only auto-detecting factory. Resolves the base model, infers its
 * family, and delegates to the matching family builder. Mirrors the way
 * the per-family entry points (`ai-retry/<family>-model`) are wired, so a
 * single import can build a retryable model for any family in tests. A
 * bare string is resolved as a language model.
 */
